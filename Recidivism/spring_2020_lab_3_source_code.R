...
####ADDED DPRICE 20150409 

#### FUNCTION FOR COMPARING THE MEANS OF TWO GROUPS
compareMeans = function(x,groups, diff=TRUE, output=TRUE){
  
  #cat("Mean in each group \n",aggregate(x,list(groups),mean),"Difference in means")
  results<-aggregate(x,list(groups),FUN=mean)
  diff<-aggregate(x,list(groups),mean)[1,2]-aggregate(x,list(groups),mean)[2,2]
  colnames(results)<-c(deparse(substitute(groups)),deparse(substitute(x)))
  if(output){cat(paste("Difference in means: ",
            round(diff,digits=2),"\n"))}
...
#### FUNCTIONS FOR COMPARING PROPORTIONS BETWEEN TWO GROUPS
freqtable = function(x){
  results=table(x)/length(x)
  return(results)
}

compareProportions = function(x, groups, diff=TRUE){
  results<-aggregate(x,by=list(groups),FUN=freqtable)
  diff<-results[1,2][1]-results[2,2][1]
  colnames(results)<-c(deparse(substitute(groups)),deparse(substitute(x)))
  cat(paste("Difference in proportions: ",
            round(diff,digits=2),"\n"))
  return(results) 
}

##### OTHER STUFF from KLM
